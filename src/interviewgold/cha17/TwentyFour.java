package interviewgold.cha17;

/**
 * @author :  PF_23
 * @Description : 最大子矩阵
 * 假设有以下矩阵，为了图示的美观，下标从1开始。其中假设某子矩阵为[r1, c1, r2, c2][r1,c1,r2,c2]。
 * <p>
 * 那么，子矩阵的和即为
 * (a[r1][c1] + a[r1+1][c1] + ... + a[r2][c1]) + ... +
 * (a[r1][j] + a[r1+1][j] + ... + a[r2][j]) + ... +
 * (a[r1][c2] + a[r1+1][c2] + ... + a[r2][c2])。
 * 其中 (a[r1][j] + a[r1+1][j] + ... + a[r2][j])(a[r1][j]+a[r1+1][j]+...+a[r2][j]) 即为从r1行到r2行第j列上的元素之和。
 * 这个值可以通过预先求得的列上的前缀和通过O(1)O(1)时间求得。可以认为我们在y轴方向上压缩矩阵，变成一维数组。
 * 该一维数组的每个元素即为列上的某个区域和。然后问题就转化成求一维数组的最大子数组和。这里矩阵的上边和矩阵的下边是变化的，
 * 也就是需要m大小的二重循环，然后求一维数组的最大子数组和需要O(n)O(n)时间，因此总的时间复杂度为O(m * m * n)O(m∗m∗n)。
 * @date : 2020/04/09.
 */
